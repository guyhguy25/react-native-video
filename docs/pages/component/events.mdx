...
| currentOffsetFromLive | number | Float value indicating the latency of the live stream in seconds                                  |
| currentLiveTimestamp  | number | The exact timestamp from the latest #EXT-X-PROGRAM-DATE-TIME in the HLS playlist                  |
...
```javascript
{
  ...
  currentOffsetFromLive: 882.8
  currentLiveTimestamp: 1708171200000
}
```
